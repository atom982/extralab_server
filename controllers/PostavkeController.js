      ...
      if (err) {
        ...
        tehnologijaAparata.find({}).exec(function (err, tehno) {
          if (err) {
            ...
            grupaTesta.find({}).populate('sekcija').exec(function (err, grupa) {
              if (err) {
                console.log("Greška:", err)
                // res.json({ success: false, message: err})
              } else {
...
postavkeController.lokacijaList = function (req, res) {
  if (mongoose.connection.readyState != 1) {
    ...
    Lokacija.find({ site: mongoose.Types.ObjectId(req.query.site) }).populate('site').exec(function (err, lokacije) {
      if (err) {
        console.log("Greška:", err)
        res.json({
          success: false,
          ...
          if (err) {
            ...
            Sekcija.find({}).exec(function (err, sekcije) {
              if (err) {
                ...
                Analyser.find({ site: mongoose.Types.ObjectId(req.query.site) }).exec(function (err, aparati) {
                  if (err) {
                    console.log("Greška:", err)
                    // res.json({ success: false, message: err}) 
                  } else {
...
postavkeController.grupaMountList = function (req, res) {
  grupaTesta.find({}).populate('sekcija').exec(function (err, grupe) {
    if (err) {
      console.log("Greška:", err)
      res.json({
        success: false,
...
postavkeController.aparatList = function (req, res) {
  if (mongoose.connection.readyState != 1) {
    ...
    Analyser.find({ site: mongoose.Types.ObjectId(req.query.site) }).populate('site').exec(function (err, aparati) {
      if (err) {
        console.log("Greška:", err)
        res.json({
          success: false,
...
postavkeController.aparatListAll = function (req, res) {
  if (mongoose.connection.readyState != 1) {
    ...
    Analyser.find({}).populate('site').exec(function (err, aparati) {
      if (err) {
        console.log("Greška:", err)
        res.json({
          success: false,
    ...
    Analyser.find({
      ...
      if (err) {
        ...
        AnaAssays.find({
          ...
          disabled: false,
          site: mongoose.Types.ObjectId(req.body.site)
        }).populate('test').exec(function (err, aptests) {
          if (err) {
            console.log("Greška:", err)
            res.json({
              success: false,
...
postavkeController.aptestListAll = function (req, res) {
  if (mongoose.connection.readyState != 1) {
    ...
    AnaAssays.find({     
      disabled: false,
      site: mongoose.Types.ObjectId(req.body.site)
    }).populate('test aparat').exec(function (err, aptests) {
      if (err) {
        console.log("Greška:", err)
        res.json({
          success: false,
...
postavkeController.aptestShow = function (req, res) {
  Results.findOne({
    id: req.body.id_u
  }).populate('rezultati.labassay rezultati.rezultat.anaassay').exec(function (err, rezultat) {
    if (err) {
      console.log("Greška:", err)
      res.json({
        success: false,
...
postavkeController.referentneList = function (req, res) {
  if (mongoose.connection.readyState != 1) {
    ...
    AnaAssays.findOne({
      ...
      site: mongoose.Types.ObjectId(req.body.site)
    }).populate('test aparat').exec(function (err, aptest) {
      if (err) {
        console.log("Greška:", err)
        res.json({
          success: false,
...
postavkeController.refAnalitSave = function (req, res) {
  if (mongoose.connection.readyState != 1) {
    ...
    AnaAssays.findOne({
      _id: mongoose.Types.ObjectId(req.body.test),
      site: mongoose.Types.ObjectId(req.body.site)
    }).populate('test aparat').exec(function (err, assay) {
      if (err) {
        console.log("Greška:", err)
        res.json({
          success: false,
...
postavkeController.kontroleDetalji = function (req, res) {
  if (mongoose.connection.readyState != 1) {
    ...
    Kontrole.findOne({
      _id: req.body._id
    }).populate().exec(function (err, kontrola) {
      if (err) {
        console.log("Greška:", err)
        res.json({
          success: false,
...
postavkeController.paneliList = function (req, res) {
  if (mongoose.connection.readyState != 1) {
    ...
    Paneli.find({ site: mongoose.Types.ObjectId(req.query.site) }).lean().exec(function (err, paneli) {
      if (err) {
        console.log("Greška:", err)
        res.json({
          success: false,
...
postavkeController.sajtoviSave = function (req, res) {
  ...
  var writestream = fs.createWriteStream(config.site_path + req.body.sifra + '.jpeg')
  var readstream = fs.createReadStream(config.multer_temp + req.body.sifra + '.jpeg').pipe(writestream)

  writestream.on('finish', function () {
    var token = req.body.token || req.query.token
    jwt.verify(token, process.env.JWT_SECRET, function (err, decoded) {
      if (err) {
        res.json({ status: "error", message: 'Niste autorizirani', data: null })
      } else {
...
postavkeController.sajtoviList = function (req, res) {
  if (mongoose.connection.readyState != 1) {
    ...
    Site.find({}).lean().exec(function (err, sajtovi) {
      if (err) {
        console.log("Greška:", err)
        res.json({
          success: false,
...
postavkeController.getSajt = function (req, res) {
  if (mongoose.connection.readyState != 1) {
    ...
    Site.findOne({ _id: req.params.id }).lean().exec(function (err, sajt) {
      if (err) {
        console.log("Greška:", err)
        res.json({
          success: false,
    ...
    AnaAssays.findOne({ _id: req.body.id }).exec(function (err, assay) {
      if (err) {
        ...
        if (assay) {
          ...
          assay.save()
          res.json({ success: true, message: 'Ok', data: 'Tip je uspješno izmjenjen.' })
        } else {
...
postavkeController.userSajtoviList = function (req, res) {
  if (mongoose.connection.readyState != 1) {
    ...
    User.findOne({ email: req.body.decoded.user }).populate('sites').exec(function (err, korisnik) {
      if (err) {
        console.log("Greška:", err)
        res.json({
          success: false,
