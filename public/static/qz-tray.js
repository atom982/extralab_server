  ...
  return {
      ...
      for (var i = 0; i < data.length; i++) {
        if (data[i].constructor === Object) {
          if (
            (!data[i].format &&
              data[i].type &&
              (data[i].type.toUpperCase() !== "RAW" &&
                data[i].type.toUpperCase() !== "DIRECT")) || //unspecified format and not raw -> assume file
            (data[i].format &&
              (data[i].format.toUpperCase() === "FILE" ||
                (data[i].format.toUpperCase() === "IMAGE" &&
                  !(
                    data[i].data.indexOf("data:image/") === 0 &&
                    data[i].data.indexOf(";base64,") !== 0
                  )) ||
    ...
    serial: {
      ...
      sendData: function(port, data, properties) {
        if (properties != null) {
          _qz.log.warn(
            "Properties object is deprecated on sendData calls, use openPort instead."
          );
        }
